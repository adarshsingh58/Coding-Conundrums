package com.datastructures.Arrays.conundrums.rotation;
...
public class MaximumHammingDistanceInJumbledArray {
	...
	/**
	 * @Pseudo
	 * >The idea is that if the similar item count is less than dissimilar items, they can be moved to those dissimilar places
	 ...
	 * 
	 * @Analysis
	 * findDuplicate_ViaMap() takes O(n) time  and findSimilarElements() takes O(n) times which combinly is O(n) space complexity
	...
	private int findSimilarElements(int[] inputArr) {
		...
		Map<Integer,Integer> map=FindingDuplicateElements.findDuplicate_ViaMap(inputArr);
