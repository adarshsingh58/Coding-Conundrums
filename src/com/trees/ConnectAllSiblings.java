...
/**
 * Given the root to a binary tree where each node has an additional pointer
 * called sibling (or next), connect the sibling pointer to next node in the
 * same level. Last node in each level should point to the first node of next
 * level in the tree.
 * 
 * 
 * DO it in O(1) mem complexity
 */
public class ConnectAllSiblings {
	...
	private static void connectAllSiblings(Node root) {
		...
		while (!levelDataQ.isEmpty() || !mainQ.isEmpty()) {
			if (!levelDataQ.isEmpty()) {
				...
				curr.sibling=levelDataQ.peek();
				if(levelDataQ.size()==0) {
					curr.sibling=mainQ.peek();//This will connect last node in level to next node in next level
				}
			} else {
	...
	/*
	 * The change here is that, in Node impl, we have added 
	 * a sibling Node, that will point to the right sibing in same level
	 * */
